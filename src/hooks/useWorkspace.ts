...
const createGitReadme = (workspace: WorkspaceMeta) => `# ${workspace.name}
...
\`\`\`bash
git init
git add ${workspace.collectionsFileName} ${WORKSPACE_MANIFEST_FILE} README.md .gitignore
    ...
    async (workspace: WorkspaceMeta, nextCollections: Collection[]) => {
      ...
      try {
        ...
        if (workspace.gitEnabled) {
          await writeTextFile(directoryHandle, "README.md", createGitReadme(workspace));
