...
export function CollectionsPanel({
  ...
  const openNewRequestDialog = (collection: Collection) => {
    setTargetCollection(collection);
    setNewRequestName(`New Request ${collection.requests.length + 1}`);
    setNewRequestOpen(true);
  };

  const openNewFolderDialog = (collection: Collection) => {
    setTargetCollection(collection);
    setNewFolderName("");
    setNewFolderOpen(true);
  };

  const openNewJsFileDialog = (collection: Collection) => {
    setTargetCollection(collection);
  ...
  const handleCreateRequest = () => {
    ...
    onAddRequestToCollection(targetCollection.id, createEmptyRequest(newRequestName.trim()));
  ...
  const openRunner = (collection: Collection) => {
    if (collection.requests.length === 0) {
      toast({
        title: "Nothing to Run",
        description: "This collection has no saved requests.",
        variant: "destructive",
      });
      return;
    }
    ...
    setRunCollectionName(collection.name);
    setRunResults([]);
    setRunnerFilter("all");
  ...
  const handleShareCollection = async (collection: Collection) => {
    ...
    if (copied) {
      toast({
        ...
        description: "Collection JSON copied to clipboard.",
      });
      return;
    }

    downloadTextFile(`${sanitizeFileName(collection.name)}-share.json`, payload, "application/json");
    toast({
  ...
  const handleGenerateDocs = (collection: Collection) => {
    const markdown = [
      `# ${collection.name}`,
      "",
      collection.description || "_No description provided._",
      "",
      `Total requests: ${collection.requests.length}`,
      "",
      "## Requests",
      "",
      ...collection.requests.flatMap((request, index) => {
        const sections = [
    ...
    openArtifactDialog(
      `Generated Docs: ${collection.name}`,
      markdown,
      `${sanitizeFileName(collection.name)}-docs.md`,
      "text/markdown"
    );
  };
  ...
  const handleRevealInFinder = (collection: Collection) => {
    ...
    downloadTextFile(`${sanitizeFileName(collection.name)}.json`, payload, "application/json");
    toast({
      title: "Collection Exported",
      description: "Downloaded collection JSON (Finder reveal equivalent in web mode).",
    });
  };
  ...
  const handleOpenInTerminal = async (collection: Collection) => {
    const script = [
      ...
      `# ${collection.name}`,
      "",
      ...collection.requests.flatMap((request) => [requestToCurl(request), ""]),
    ].join("\n");

    const copied = await copyToClipboard(script);
    openArtifactDialog(
      `Terminal Script: ${collection.name}`,
      script,
      `${sanitizeFileName(collection.name)}.sh`,
      "text/x-shellscript"
    );
    toast({
  ...
  const renderRequestTree = (
    ...
    nodes.map((node) => {
      ...
      const expanded = isFolderExpanded(collectionId, node.key);

      return (
        <Collapsible
          key={`${collectionId}:${node.key}`}
          open={expanded}
          onOpenChange={(open) => setFolderExpanded(collectionId, node.key, open)}
        >
          <CollapsibleTrigger asChild>
            <Button
              ...
              size="sm"
              className={cn(
                "w-full justify-start gap-2 h-8 px-2 text-xs overflow-hidden",
                depth > 0 && "pl-3"
              )}
            >
              {expanded ? (
          ...
          <CollapsibleContent className="pl-4 space-y-0.5 overflow-hidden">
            {renderRequestTree(collectionId, node.children, depth + 1)}
          </CollapsibleContent>
        </Collapsible>
      );
    });
                ...
                >
                  ...
                  <CollapsibleContent className="pl-6 pr-1 space-y-0.5 overflow-hidden">
                    ...
                    {collection.requests.length === 0 ? (
                      ...
                      >
                        {renderRequestTree(collection.id, buildCollectionRequestTree(collection.requests))}
                      </SortableContext>
                    )}
                  </CollapsibleContent>
                </Collapsible>
