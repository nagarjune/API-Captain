...
export interface BuilderSettings {
  ...
  proxyHost: string;
  proxyPort: string;
  featureApiSpec: boolean;
  featureGit: boolean;
  featureFileExplorer: boolean;
  featureShowApiCaptainJson: boolean;
  enableBeta: boolean;
...
export const BUILDER_SETTINGS_UPDATED_EVENT = "api-captain-settings-updated";
...
export const DEFAULT_BUILDER_SETTINGS: BuilderSettings = {
  ...
  proxyHost: "",
  proxyPort: "8080",
  featureApiSpec: false,
  featureGit: false,
  featureFileExplorer: false,
  featureShowApiCaptainJson: false,
  enableBeta: false,
...
export const getStoredBuilderSettings = (): BuilderSettings => {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<BuilderSettings> & { featureShowBrunoJson?: boolean };
    const migratedShowConfig =
      typeof parsed.featureShowApiCaptainJson === "boolean"
        ? parsed.featureShowApiCaptainJson
        : Boolean(parsed.featureShowBrunoJson);
    return {
      ...DEFAULT_BUILDER_SETTINGS,
      ...parsed,
      featureShowApiCaptainJson: migratedShowConfig,
    };
  } catch {
    return DEFAULT_BUILDER_SETTINGS;
  }
...
export const saveBuilderSettings = (settings: BuilderSettings): void => {
  ...
  window.dispatchEvent(new Event(BUILDER_SETTINGS_UPDATED_EVENT));
};
